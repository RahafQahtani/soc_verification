...
bit [31:0] i2c_addr = 32'h20000300;
bit [3:0] offset = 4; 
...
//  prescale the SCL clock
    `uvm_do_with(req,
                 { op_type == wb_write ; 
                   addr == i2c_addr; //Clock Prescale register lo-byte 
    ...
    `uvm_do_with(req,
                 { op_type == wb_write ; 
                   addr == i2c_addr+1*offset; //Clock Prescale register HI-byte 
    ...
    `uvm_do_with(req,
                 { op_type == wb_write ; 
                   addr == i2c_addr+2*offset; //i2c control register (ctr)
...
`uvm_do_with(req,
                 { op_type == wb_write ; 
                   addr == i2c_addr+3*offset; //i2c transmit register
      ...
      `uvm_do_with(req,
                 { op_type == wb_write ; 
                   addr == i2c_addr+4*offset; //i2c command register
...
////////////              
  ...
  // this  additional polling transaction is "necessary" to bypass the initial dout value of 8'b0000_0000
     `uvm_do_with(req,
                 { op_type == wb_read ; 
                   addr == i2c_addr+4*offset; //i2c status register
                   valid_sb == 0;  //indicate to scoreboard that this is configuration command
                 })
  count_polling =0;
  ...
  if (!req.dout[7] && !req.dout[1]) begin 
    // check for ACK && TIP both bits should be de-asserted
     `uvm_info (get_type_name(), "break pooling",UVM_HIGH);
...
class i2c_write_to_wrong_addr_seq extends wb_base_seq;
 ...
 // sendign heaeder byte 
     `uvm_do_with(req,
      ...
      `uvm_do_with(req,
                 { op_type == wb_write ; 
                   addr == i2c_addr+3*offset; //i2c command register
                   din == 8'b1001_0000; //sta & wr
                   valid_sb == 0;  //indicate to scoreboard that this is configuration command
                 })
